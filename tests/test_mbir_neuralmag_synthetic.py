...
def _build_vortex_disc_state():
    ...
    disc_mask = ((yy - cy) ** 2 + (xx - cx) ** 2) <= (0.32 * n_x) ** 2
...
def _relax_to_m_true(state):
    ...
    llg.relax(tol=1e9, dt=3e-12)
    m_true = np.asarray(state.m.to_cell().tensor)
    return m_true


def _neuralmag_to_projected_adapter(state):
    m_true = np.asarray(state.m.to_cell().tensor)
    rho = np.asarray(state.rho.tensor)
    m_true = m_true * rho[..., None]
    m_projected = np.sum(m_true[..., :2], axis=0)
...
def test_neuralmag_vortex_ground_truth_smoke():
    ...
    converted = _neuralmag_to_projected_adapter(state)
