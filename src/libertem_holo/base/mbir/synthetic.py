...
All spatial arrays use LiberTEM ordering ``(X, Y, Z)``.
...
def _as_center_yx(shape_xyz: tuple[int, int, int], center_yx: tuple[float, float] | None) -> tuple[float, float]:
...
def _spatial_grid(shape_xyz: tuple[int, int, int], dtype) -> tuple[jnp.ndarray, jnp.ndarray]:
    nx, ny, _ = shape_xyz
    xx, yy = jnp.meshgrid(
...
def soft_disc_support(
    ...
    *,
    ...
    """Return a differentiable soft disc support field with shape ``(X, Y, Z)``."""
    if radius <= 0:
        raise ValueError(f"radius must be positive, got {radius}")
    if edge_width <= 0:
        raise ValueError(f"edge_width must be positive, got {edge_width}")

    yy, xx = _spatial_grid(shape_xyz, dtype=dtype)
    cy, cx = _as_center_yx(shape_xyz, center_yx)
    r = jnp.hypot(yy - cy, xx - cx)
    soft_2d = jax.nn.sigmoid((radius - r) / edge_width)
    return jnp.broadcast_to(soft_2d[..., None], shape_xyz)


def uniform_magnetization(
    shape_xyz: tuple[int, int, int],
    *,
    support_xyz: jnp.ndarray | None = None,
    direction_xyz: tuple[float, float, float] = (1.0, 0.0, 0.0),
    magnitude: float = 1.0,
    dtype=jnp.float32,
) -> jnp.ndarray:
    """Return a uniform magnetization field of shape ``(X, Y, Z, 3)``."""
    if magnitude < 0:
    ...
    support = jnp.ones(shape_xyz, dtype=dtype) if support_xyz is None else jnp.asarray(support_xyz, dtype=dtype)
    support = jnp.broadcast_to(support, shape_xyz)
...
def vortex_magnetization(
    ...
    *,
    ...
    base = jnp.broadcast_to(base[..., None, :], shape_xyz + (3,))
...
def domain_wall_magnetization(
    ...
    *,
    ...
    base = jnp.broadcast_to(base_line[:, None, None, :], shape_xyz + (3,))
